<?php

// --- INISIALISASI SESSION (DEFAUT) ---
// Pastikan status defaultnya FALSE jika belum pernah diatur
if (!isset($_SESSION['tampil_jurusan'])) {
    $_SESSION['tampil_jurusan'] = false; // Default: Sembunyikan
}

// --- LOGIKA TOGGLE ---
if (isset($_GET['toggle']) && $_GET['toggle'] == 'jurusan') {
    // Jika tombol diklik (ada parameter 'toggle=jurusan'), 
    // ubah status menjadi kebalikannya.
    
    $_SESSION['tampil_jurusan'] = !$_SESSION['tampil_jurusan'];
    $_SESSION['tampil_daftar'] = false; // Sembunyikan tabel siswa
    $_SESSION['tampil_add_j'] = false;
    $_SESSION['tampil_kebutuhan'] = false;
    $_SESSION['tampil_add_k'] = false;
    
    // REDIRECT BERSIH: Hapus parameter 'toggle' dari URL
    // Ini penting agar status tidak berubah lagi jika user me-refresh halaman.
    header('Location: beranda_admin.php');
    exit;
}


$tampilkan_tabel_jurusan = $_SESSION['tampil_jurusan'] ?? false;
if ($tampilkan_tabel_jurusan) {
    // Panggil fungsi yang baru kita buat untuk mengambil data
    $daftar_jurusan = getDaftarJurusan($DBH); 
}
?>